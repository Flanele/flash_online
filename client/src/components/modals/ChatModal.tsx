...
const ChatModal: React.FC<ChatModalProps> = ({
  ...
  const { data: friends, isLoading: isFriendsLoading } =
    useSearchFriendsQuery(searchTerm);
          ...
          {selectedFriend ? (
            <>
              <div className="flex items-center p-4 border-b border-gray-300">
                {friends &&
                friends.find((user) => user.id === selectedFriend) ? (
                  <>
                    {friends.find((user) => user.id === selectedFriend)
                      ?.avatar_url ? (
                      <img
                      ...
                      <div className="w-12 h-12 rounded-full bg-nav text-white flex items-center justify-center mr-4">
                        {friends
                          .find((user) => user.id === selectedFriend)
              ...
              >
                <div ref={topSentinelRef} className="h-[1px]" />
                ...
                {(isMessagesLoading || isMessagesFetching) && (
                  <div className="flex items-center justify-center space-x-2 py-2">
                    <div className="w-8 h-8 border-4 border-t-transparent border-purple-500 rounded-full animate-spin" />
