...
class FavoriteController {
  async getAllFavorites(req, res, next) {
    try {
      ...
      return next(
        ApiError.badRequest("Не удалось загрузить избранные пользователя"),
      );
    }
  }
  ...
  async addGameToFavorites(req, res, next) {
    try {
      ...
      const existingFavoriteGame = await FavoriteGame.findOne({
        where: { userId, gameId },
      });
      ...
      return res
        .status(201)
        .json({ message: "Игра успешно добавлена в избранное" });
    } catch (error) {
      return next(
        ApiError.badRequest(
          "Что-то пошло не так при добавлении в избранное...",
        ),
      );
    }
  }
  ...
  async removeGameFromFavorites(req, res, next) {
    try {
      ...
      return next(
        ApiError.badRequest(
          "Что-то пошло не так при удалении из избранного...",
        ),
      );
    }
  }
  ...
  async checkIsGameFavorite(req, res, next) {
    try {
      ...
      return next(
        ApiError.badRequest("Не удалось проверить статус избранного"),
      );
    }
  }
}
