...
class FriendController {
  async getFriendsList(req, res, next) {
    try {
      ...
      console.log(error);
      return next(
        ApiError.internal("Не получилось получить список друзей пользователя"),
      );
    }
  }
  ...
  async addFriend(req, res, next) {
    try {
      ...
      if (existingFriendship) {
        return next(
          ApiError.badRequest("Вы уже отправили запрос этому пользователю"),
        );
      }
      ...
      console.log(error);
      return next(
        ApiError.badRequest("Ошибка при отправлении запроса в друзья"),
      );
    }
  }
