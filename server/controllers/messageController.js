...
class MessageController {
  async getAllMessagesWithUser(req, res, next) {
    try {
      ...
      return next(
        ApiError.badRequest(
          "Не получилось загрузить историю переписки с пользователем",
        ),
      );
    }
  }
  ...
  async getUnreadMessagesByUser(req, res, next) {
    try {
      ...
      const unreadMessagesCount = await Message.count({
        where: { senderId: senderId, receiverId: userId, read: false },
      });
      ...
      console.log(error);
      return next(
        ApiError.badRequest("Ошибка при получении непрочитанных сообщений"),
      );
    }
  }
  ...
  async editMessage(req, res, next) {
    try {
      ...
      if (!text || text.trim() === "") {
        return next(
          ApiError.badRequest("Текст сообщения не может быть пустым"),
        );
      }
      ...
      if (!message) {
        return next(
          ApiError.badRequest("Сообщение не найдено или доступ запрещён"),
        );
      }
      ...
      console.log(
        `Сообщение ${id} пользователя ${userId} обновлено: "${oldText}" -> "${text}"`,
      );
