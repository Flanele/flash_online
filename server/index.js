...
const allowOrigins = ["http://localhost:5173", process.env.FRONTEND_URL].filter(
  Boolean,
);
...
io.on("connection", (socket) => {
  ...
  socket.on("login", (userId) => {
    ...
    console.log(
      `Socket для userId ${userId} зарегистрирован с ID ${socket.id}`,
    );
  ...
  socket.on("delete_message", ({ userId, id, senderId }) => {
    if (userSockets[userId]) {
      io.to(userSockets[userId]).emit("delete_message", {
        userId,
        id,
        senderId,
      });
  ...
  socket.on("edit_message", ({ id, text, userId, senderId }) => {
    if (userSockets[userId]) {
      io.to(userSockets[userId]).emit("edit_message", {
        id,
        text,
        userId,
        senderId,
